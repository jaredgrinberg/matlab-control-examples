function  model = model_cartpole_2_link()
    ...
    model.Bmat = [1;0;0];  % Actuation matrix (force only applied to cart)
    ...
    % Define pole 2 (body 3)
    nb = nb + 1;
    model.parent(nb) = nb - 1;     % Connected to first pole
    model.jtype(nb) = {'Ry'};      % Rotary joint around Y axis
    model.Xtree(nb) = {plux(eye(3), [0; 0; -len_pole])};  % Spatial transform from cart
    model.I(nb) = { mcI( m_pole, [0 0 -0.5*len_pole], diag([0 Iyy_pole 0]) ) };
    model.appearance.body{nb} = ...
        { 'colour', green, 'cyl', [0 0 0; 0 0 -len_pole], 0.02};
