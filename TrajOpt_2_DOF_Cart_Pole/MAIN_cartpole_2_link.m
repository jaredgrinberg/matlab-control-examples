...
p.robot = model_cartpole_2_link();  % Get robot model
nq = p.robot.NB;    % Number of configuration variables
nu = 1;             % Number of control inputs
hor = 100;           % Horizon length (timesteps)
dt = 0.04;          % Timestep size (seconds)
Umax = 35;          % Maximum control force (N)
...
q0 = [0; 0; 0];         % Initial position [cart; pole1; pole2]
dq0 = [0; 0; 0];        % Initial velocities
qd = [0; pi; 0];       % Target: both poles up
X0 = [q0; dq0];     % Initial state
Xd = [qd; 0; 0; 0];     % Desired state
...
for kk = 1:hor
   ...
   elseif kk == hor
       ...
       % If it cant reach that - infeasible, just make bounds
       final_tolerance = .5;
       opti.subject_to(X(:,kk) <= Xd + final_tolerance);
       opti.subject_to(X(:,kk) >= Xd - final_tolerance);
...
%% Save results
save('cartpole.mat', 't', 'state', 'control','dt','hor')  % state will be 6x50 instead of 4x50
